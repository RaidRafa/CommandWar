...
// {
    ...
    public class BarcoInimigoCombate : MonoBehaviour
    {
        ...
        void AndarDevagar()
        {
            Vector3 desiredVelocity = transform.forward * maxSpeed * 0.5f;//0.3f
            FisicaDoBarquinho.ApplyForceToReachVelocity(rb, desiredVelocity, acceleration);
        }

        void GirarLeve()
        {
            float torque = Mathf.Sin(Time.time) * steerPower * 0.9f;//0.5f
        ...
        void AndarReto()
        {
            Vector3 desiredVelocity = transform.forward * maxSpeed * 0.9f;//0.6f
        ...
        void TentarDisparar()
        {
            ...
            bool estaNaLateral = angle > 60f && angle < 100f;
